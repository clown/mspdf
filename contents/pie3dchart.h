#ifndef FAML_PDF_CONTENTS_PIE3DCHART_H
#define FAML_PDF_CONTENTS_PIE3DCHART_H
	...
	namespace pdf {
		namespace contents {
			...
			public:
				...
				explicit pie3dchart(const coordinate& o, double w, double h) :
			...
			private:
				...
				bool put_data(ostream_type& out, double ox, double oy) {
					...
					if (this->width() < this->height()) {
						double width = this->width() - legend_ - 3*margin_;
						w = width / 2.0;
						h = width * (7.0 / 12.0) * (17.0 / 21.0) / 2.0;
						d = width * (7.0 / 12.0) * (4.0 / 21.0);
					}else{
						double height = this->height() - title_h_ - 3*margin_;
					...
					for (size_type i = 0; i < this->in_->labels().size(); ++i) {
						...
						if (start_angle > 0 && end_angle < 0) {
							detail::draw_angle_line(out, ox+(this->width()-margin_-legend_)/2.0, oy+margin_+h+d, w, h, d, 0);
							detail::draw_val_arc_rev(out, ox+(this->width()-margin_-legend_)/2.0, oy+margin_+h, w, h, 0, end_angle, true);
							detail::draw_angle_line(out, ox+(this->width()-margin_-legend_)/2.0, oy+margin_+h, w, h, -d, 0, true);
							out << LITERAL("b") << std::endl;
						}
						if (start_angle > -180 && end_angle < -180) {
							detail::draw_angle_line(out, ox+(this->width()-margin_-legend_)/2.0, oy+margin_+h+d, w, h, d, start_angle);
							detail::draw_val_arc_rev(out, ox+(this->width()-margin_-legend_)/2.0, oy+margin_+h, w, h, start_angle, -180, true);
							detail::draw_angle_line(out, ox+(this->width()-margin_-legend_)/2.0, oy+margin_+h, w, h, -d, -180, true);
							out << LITERAL("b") << std::endl;
						}
						detail::draw_angle_line(out, ox+(this->width()-margin_-legend_)/2.0, oy+margin_+h+d, w, h, d, start_angle);
						detail::draw_val_arc_rev(out, ox+(this->width()-margin_-legend_)/2.0, oy+margin_+h, w, h, start_angle, end_angle, true);
						detail::draw_angle_line(out, ox+(this->width()-margin_-legend_)/2.0, oy+margin_+h, w, h, -d, end_angle, true);
						out << LITERAL("b") << std::endl;
					...
					for (size_type i = 0; i < this->in_->labels().size(); ++i) {
						...
						detail::draw_val_arc_rev(out, ox+(this->width()-margin_-legend_)/2.0, oy+margin_+h+d, w, h, start_angle, end_angle);
						out << fmt(LITERAL("%f %f l")) % (ox+(this->width()-margin_-legend_)/2.0) % (oy+margin_+h+d) << std::endl;
						out << LITERAL("b") << std::endl;
						out << LITERAL("Q") << std::endl;
						start_angle = end_angle;
					}
...
#endif // FAML_PDF_CONTENTS_PIE3DCHART_H
