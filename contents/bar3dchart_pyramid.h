#ifndef FAML_PDF_CONTENTS_BAR3DCHART_PYRAMID_H
#define FAML_PDF_CONTENTS_BAR3DCHART_PYRAMID_H
...
namespace faml {
	namespace pdf {
		namespace contents {
			...
			>
			class bar3dchart_pyramid : public chart3d<DataMap, CharT, Traits> {
			public:
				...
				bar3dchart_pyramid() : super() {}
				
				bar3dchart_pyramid(const coordinate& o, double w, double h) :
					super(o, w, h) {}
				
				virtual ~bar3dchart_pyramid() throw() {}
				...
				virtual bool put_data(ostream_type& out, double ox, double oy) {
					...
					for (size_type i = 0; i < this->in_->labels().size(); ++i) {
						for (size_type j = 0; j < this->in_->size(); ++j) {
							...
							out << fmt(LITERAL("%f %f l")) % (ox) % (oy + z_h) << std::endl;
							...
							out << fmt(LITERAL("%f %f m")) % (ox + width / 2.0) % (oy + height) << std::endl;
							out << fmt(LITERAL("%f %f l")) % (ox + width + z_w) % (oy + 2*z_h) << std::endl;
							out << fmt(LITERAL("%f %f l")) % (ox + width) % (oy + z_h) << std::endl;
							out << LITERAL("b") << std::endl;
...
#endif // FAML_PDF_CONTENTS_BAR3DCHART_PYRAMID_H
